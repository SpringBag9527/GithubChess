...
//    private static final Dimension CHESSGRID_SIZE = new Dimension(1080 / 4 * 3 / 8, 1080 / 4 * 3 / 8);
    ...
    private static final Color[] BACKGROUND_COLORS = {Color.WHITE, Color.PINK};//Color.BLACK};
    /**
     * handle click event
     */
    ...
    private ArrayList<ChessboardPoint> canMoveToList = new ArrayList<>();
    ...
    protected ChessComponent(ChessboardPoint chessboardPoint, Point location, ChessColor chessColor, ClickController clickController, int size, char name, String chesspath) {
...
//            System.out.printf("Mouse Enter (%d,%d)\n",chessboardPoint.getX(),chessboardPoint.getY());
            this.setInArea(true);
            this.repaint();
        }
        //鼠标退出
        if (e.getID() == MouseEvent.MOUSE_EXITED){
//            System.out.printf("Mouse Exit (%d,%d)\n",chessboardPoint.getX(),chessboardPoint.getY());
            this.setInArea(false);
            repaint();
        }
    }
    ...
    public abstract boolean canReach(ChessComponent[][] chessboard, ChessboardPoint destination);
    /**
     * 这个方法主要用于加载一些特定资源，如棋子图片等等。
     *
     * @throws IOException 如果一些资源找不到(如棋子图片路径错误)，就会抛出异常
     */
    public abstract void loadResource(String chesspath) throws IOException;
...
//        System.out.printf("repaint chess [%d,%d]\n", chessboardPoint.getX(), chessboardPoint.getY());
        ...
        //其他标志
        if (isCanMove()){
            squareColor = new Color(200,100,100);
            g.setColor(squareColor);
//            g.fill3DRect(0,0,this.getWidth(),this.getHeight(),true);
            g.fillRect(0,0,this.getWidth(),this.getHeight());
        }
        if (isInArea()){
            squareColor = new Color(112,128,144);
            g.setColor(squareColor);
    ...
    public void setBackGroundColors(Color color0,Color color1){
        this.BACKGROUND_COLORS[0]=color0;
        this.BACKGROUND_COLORS[1]=color1;
    }
}
